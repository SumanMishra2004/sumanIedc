...
import { exportCertificatesToCSV } from "@/lib/research/certificateApi"
...
export function ExportDialog({ triggerButton }: ExportDialogProps) {
  ...
  const handleExport = async () => {
    setIsExporting(true)
    try {
      await exportCertificatesToCSV(filters);
  ...
  const handleFiltersChange = (newFilters: Partial<CertificateFilters>) => {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Export certificates to CSV format. You can apply filters to export specific data.
          </DialogDescription>
        </DialogHeader>
