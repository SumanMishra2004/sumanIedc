...
const statusChartConfig: ChartConfig = {
  ...
  PRESENTED: { label: "Presented", color: "var(--chart-5)" },
  PUBLISHED: { label: "Published", color: "var(--chart-4)" },
}

const trendChartConfig: ChartConfig = {
  count: { label: "Conferences", color: "var(--chart-2)" },
}

export default function AdminConferenceStats() {
  ...
  const fetchStats = useCallback(async () => {
    ...
    if (response.data) {
      ...
      toast.error("Failed to load conference statistics", {
  ...
  const publishedCount =
    stats.conferenceStatusCounts.find((s) => s.status === "PUBLISHED")?.count || 0
  const approvedCount =
    stats.conferenceStatusCounts.find((s) => s.status === "APPROVED")?.count || 0
  const presentedCount =
    stats.conferenceStatusCounts.find((s) => s.status === "PRESENTED")?.count || 0
  const submittedCount =
    stats.conferenceStatusCounts.find((s) => s.status === "SUBMITTED")?.count || 0
  const underReviewCount =
    stats.conferenceStatusCounts.find((s) => s.status === "UNDER_REVIEW")?.count || 0

  const statusData = stats.conferenceStatusCounts.map((item, idx) => ({
    status: item.status,
    count: item.count,
  ...
  const statCards = [
    ...
    {
      title: "Approved / Presented",
      value: approvedCount + presentedCount,
      subtitle: `Approved: ${approvedCount}, Presented: ${presentedCount}`,
      icon: FileCheck,
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Conferences by workflow stage</CardDescription>
          </CardHeader>
          <CardContent>
            {statusData.length > 0 ? (
              <ChartContainer
