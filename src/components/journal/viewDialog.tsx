...
import { getJournalById } from "@/lib/research/journalApi"
...
interface JournalViewDialogProps {
  ...
  setViewingJournalId: (id: string | null) => void
}
    ...
    const fetchJournal = async () => {
      ...
      try {
        ...
        if (!response.data?.journal) {
          setError("Unable to load journal details")
          setLoading(false)
          return
        }
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1">
                <DialogTitle className="text-3xl font-bold leading-tight bg-gradient-to-r from-foreground to-primary/80 bg-clip-text text-transparent">
                  {loading ? "Loading..." : error ? "Error" : journal?.title || "Untitled Publication"}
                </DialogTitle>
                ...
                {journal && (
                  <div className="flex flex-wrap items-center gap-2 mt-4">
                    ...
                    <Badge variant="outline" className={`border-2 font-medium ${getStatusBadgeColor(journal.journalStatus)}`}>
                      Journal: {journal.journalStatus}
                    </Badge>
                    <Badge variant="outline" className={`border-2 font-medium ${getTeacherBadgeColor(journal.teacherStatus)}`}>
                      Verification: {journal.teacherStatus}
                    </Badge>
                  </div>
                )}
              </div>
                          ...
                          <div className="flex flex-wrap gap-1.5">
                            {journal.keywords.map((keyword, idx) => (
                              <Badge
                                key={idx}
                                variant="outline"
                                className="px-2.5 py-0.5 text-xs font-medium hover:bg-accent/10 transition-all cursor-pointer"
                              >
                                #{keyword}
                              </Badge>
                            ))}
                      ...
                      <div className="mt-6 flex flex-col gap-2">
                        {journal.doi && (
                          <Button
                            ...
                            onClick={() => window.open(`https://doi.org/${journal.doi}`, "_blank")}
                          >
                            <ExternalLink className="w-3.5 h-3.5 mr-2 shrink-0" />
                            <span className="truncate">Open DOI: {journal.doi}</span>
                          </Button>
                        )}
                        {journal.paperLink && (
                          <Button
                            ...
                            variant="outline"
                            className="w-full justify-start text-xs"
                            onClick={() => window.open(journal.paperLink || "", "_blank")}
                          >
                            <Globe className="w-3.5 h-3.5 mr-2 shrink-0" />
                            <span className="truncate">Open Paper Link</span>
                          </Button>
                        )}
                        {journal.documentUrl && (
                          <Button
                            ...
                            variant="secondary"
                            className="w-full justify-start text-xs"
                            onClick={() => window.open(journal.documentUrl || "", "_blank")}
                          >
                          ...
                          <CardHeader className="pb-3">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                <CardDescription className="text-xs">Summary of the publication</CardDescription>
                              </div>
                            </div>
                          </CardHeader>
                          <CardContent>
                        ...
                        <CardContent>
                          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                            ...
                            <DetailCell
                              ...
                              value={`${journal.indexing} (${journal.scope})`}
                            />
                            <DetailCell
                              icon={<CheckCircle className="w-4 h-4 text-emerald-400" />}
                              label="Review & Access"
                              value={`${journal.reviewType.replace(/_/g, " ")} | ${journal.accessType.replace(/_/g, " ")}`}
                            />
                            <DetailCell
                            ...
                            {journal.impactFactor !== null && (
                              <DetailCell
                                ...
                                label="Impact Factor"
                                value={`${journal.impactFactor.toFixed(2)} ${
                                  journal.impactFactorDate
                                    ? `(${new Date(journal.impactFactorDate).getFullYear()})`
                                    : ""
                                }`}
                              />
                            )}
                            <DetailCell
                              ...
                              label="Quartile / Mode"
                              value={`${journal.quartile} | ${journal.publicationMode}`}
                            />
                            {journal.registrationFees !== null && (
                              <DetailCell
                                icon={<DollarSign className="w-4 h-4 text-red-400" />}
                                label="Registration Fees"
                                value={`₹${journal.registrationFees.toLocaleString("en-IN")}`}
                              />
                            )}
                            {journal.reimbursement !== null && (
                              <DetailCell
                                ...
                                label="Reimbursement"
                                value={`₹${journal.reimbursement.toLocaleString("en-IN")}`}
                              />
                            )}
                          </div>
                        </CardContent>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          {journal.facultyAuthors?.length > 0 && (
                            <AuthorSection
                              ...
                              authors={journal.facultyAuthors}
                            />
                          )}
                          {journal.studentAuthors?.length > 0 && (
                            <AuthorSection
                              ...
                              authors={journal.studentAuthors}
                            />
                          )}
                        </div>
                      </CardContent>
                      ...
                      <Card className="border-0 bg-gradient-to-br from-card/85 via-card to-muted/20 backdrop-blur-xl shadow-lg">
                        ...
                        <CardContent className="p-0">
                          <div className="aspect-video max-h-[70vh] w-full rounded-2xl overflow-hidden border border-muted bg-neutral-900 shadow-lg mx-auto max-w-4xl">
                            <iframe
                              src={`${journal.documentUrl}#toolbar=0&navpanes=0&scrollbar=0`}
                              className="w-full h-full"
                              title={`${journal.title} PDF Evidence`}
                              loading="lazy"
                            />
                          </div>
                        </CardContent>
