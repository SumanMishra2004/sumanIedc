...
import {
  ...
  MessageSquare, CheckCircle2, Clock, Globe,
} from "lucide-react";
...
// ─── Upload Zone (same fix as Create dialog) ────────────────────────────────────
interface UploadZoneProps {
  ...
  fileName?: string;
  hasValue: boolean;
  existingLabel?: string; // e.g. "Current image on file"
  onTrigger: () => void;
  onRemove: () => void;
  error?: string;
}
...
export default function EditBookChapterDialog({
  ...
  // ── THE FIX: hidden file inputs live OUTSIDE DialogContent ──────────────────
  ...
  const loadBookChapterData = useCallback(async () => {
    ...
    try {
      ...
      form.reset({
        ...
        bookChapterStatus: data.bookChapterStatus,
        ...
        facultyAuthorIds: data.facultyAuthors?.map((a) => a.user.id) || [],
        studentAuthorIds: data.studentAuthors?.map((a) => a.user.id) || [],
      });

      if (data.facultyAuthors) {
        setSelectedFaculty(
          data.facultyAuthors.map((a) => ({
            id: a.user.id,
            name: a.user.name || "",
            email: a.user.email || "",
      ...
      if (data.studentAuthors) {
        setSelectedStudents(
          data.studentAuthors.map((a) => ({
            id: a.user.id,
            name: a.user.name || "",
            email: a.user.email || "",
            image: a.user.image || undefined,
      ...
      if (data.publisher && !PUBLISHERS.includes(data.publisher)) {
        setShowCustomPublisher(true);
        setCustomPublisher(data.publisher);
      }
    } catch (error: any) {
  ...
  }, [bookChapterId, form, onOpenChange]);

  useEffect(() => {
    if (open && bookChapterId) {
      const id = window.setTimeout(() => void loadBookChapterData(), 0);
      return () => window.clearTimeout(id);
    }
  }, [open, bookChapterId, loadBookChapterData]);

  // ── File handlers (attached to outside inputs) ───────────────────────────
  ...
  return (
    <>
      ...
      {/*
        ...
        Same pattern as CreateDialog — inputs live outside the portal so they
        are never frozen by Radix's pointer-events / focus-trap management.
      */}
      <input
        ref={imageInputRef}
        type="file"
          ...
          {isLoading ? (
            <div className="flex items-center justify-center h-96">
              <div className="text-center space-y-3">
                ...
                <p className="text-sm text-muted-foreground">Loading chapter data…</p>
              </div>
            </div>
          ) : (
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <FormControl>
                              <Textarea
                                placeholder="Provide a concise summary of the chapter content…"
                          ...
                          <FormItem>
                            ...
                            <FormControl>
                              <UploadZone
                                ...
                                hasValue={!!field.value}
                                existingLabel="Document on file"
